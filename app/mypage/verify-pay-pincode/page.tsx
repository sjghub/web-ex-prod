...
export default function PaymentPasswordPage() {
  ...
  //이후 간편 결제 비밀번호를 받아올 수 있는 API를 작성 후 조건문으로 연결
  const checkHasPayPassword = async () => {
    try {
      ...
      if (!hasCard) {
        router.replace("/card/register");
      }
    } catch (error) {
      console.error("결제 비밀번호호 조회 실패:", error);
    }
  };
        ...
        <Card className="shadow-sm border-gray-100">
          <CardContent className="pt-6 pb-8">
            ...
            <Button
              ...
              onClick={() => router.push("/mypage/change-pay-pincode")}
            >
              완료
            </Button>
