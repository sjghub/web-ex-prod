...
export function useCardScroll() {
  useEffect(() => {
    ...
    const dots = document.querySelectorAll('.flex.justify-center.mt-4.gap-2 button');
    ...
    const updateActiveDot = () => {
      ...
      const currentPage = Math.round(scrollPosition / (scrollWidth / totalPages));
    ...
    dots.forEach((dot, index) => {
      dot.addEventListener('click', () => {
        const containerWidth = scrollContainer.clientWidth;
        const scrollTo =
          (scrollContainer.scrollWidth - containerWidth) * (index / (dots.length - 1));
    ...
    return () => {
      ...
      if (scrollContainer) scrollContainer.removeEventListener('scroll', updateActiveDot);
